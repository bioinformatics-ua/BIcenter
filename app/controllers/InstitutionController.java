...
public class InstitutionController extends Controller {
	...
	@Security.Authenticated(Secured.class)
	@CheckPermission(category = Category.INSTITUTION, needs = {Operation.UPDATE})
	public Result updateInstitution(long institutionId) {
		JsonNode formData = request().body().asJson();
		...
		if (!institution.getName().equals(institutionName) && existsInstitution(institutionName)) {
